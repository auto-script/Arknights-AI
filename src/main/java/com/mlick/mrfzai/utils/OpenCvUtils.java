...
public class OpenCvUtils {
  ...
  public static Point findImage(String sourceImg, Action templateImg, String resultImgFile) {
    ...
    if (sourceMat.width() < templateMat.width() || sourceMat.height() < templateMat.height()) {
      System.err.println("The template image is larger than the source image. Ensure that the width and/or height of " +
    ...
    if (!minMaxLocResultIsValid(minMaxLocRes)) {
      System.out.println(
          "Image find result (MinMaxLocResult) was invalid. This usually happens when the source image is covered in " +
              "one solid color.");
      return null;
    }
  ...
  public static Point loopFind(String img, boolean ifNullExit) {
    return loopFind(img, ifNullExit, null);
  }
